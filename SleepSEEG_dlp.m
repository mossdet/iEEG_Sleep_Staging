...
FileName=file{1};
header_ini = read_laydat_header(FileName);
Nch=header_ini.nChans;
channel_info.name = char(header_ini.label);
...
Nfeat=24;    
ne=0;
max_num_epochs = 48*60*2; % 48 hours, usd to be 12,5 hours or 1500 epochs of 30 seconds each
feature=zeros(Nfeat,Nch,max_num_epochs);
SleepStage=zeros(max_num_epochs,5);
night=zeros(max_num_epochs,1)==0;
for nf=1:length(file)
    FileName=file{nf};
    header_ini = read_laydat_header(FileName);
    
    num_channels=header_ini.nChans;
    channel_info.name=char(header_ini.label);
    channel_info.sensor=header_ini.chantype;
    channel_info.unit=header_ini.chanunit;
    channel_info.physmin =header_ini.physmin;
    ...
    b(a<0)=0; a(a<0)=1;
    NumSamps= header_ini.nSamples;
    Time=header_ini.study_starttime;
